<template>
  <div class="flow-design">
    ...
    <select-user :selectedUserProp.sync="nodePersons"></select-user>
  </div>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        fixedFlowData: [],
        //流程图
        flowGraph: this.flowGraphProp,

        // 节点属性值
        nodeName: '',
        nodeType: '',
        nodePersons: [],
        nodePersonType: '', // 节点处理人类型 提单人 1 ，自由选择 2，部门负责人 3，指定人 0
        nodePersonPick: '',
        nodeType: 0,
        nodeCondition: [],
        childNode: [],
    ...
    methods: {
      ...
      initG6() {
        ...
        _this.net.on('itemclick', function (ev) {
          ...
          if (_this.isNode) {
            ...
            _this.infoTitle = '节点';
            debugger;
            _this.nodeName = ev.item.get('model').label;
            _this.nodePersonType = ev.item.get('model').nodePersonType;
          } else {
            _this.infoTitle = '边';
          }
      ...
      save() {
        ...
        let flowNodes = flowData.nodes.map((item) => {
          item.nodeName = item.label;
          item.nodeId = item.id;
          return {data: item, childNode: []};
        });
        flowData.edges.forEach(edge => {
          let sourceNode = flowNodes.find(node => node.data.id === edge.source);
          let targetNode = flowNodes.find(node => node.data.id === edge.target);
//        sourceNode.childNode = [...sourceNode.childNode,targetNode];
          sourceNode.childNode.push(targetNode);
        });
        let rootNode = flowNodes.find(node => node.data.rootNode);
        debugger;
        this.$emit('update:formFlow', rootNode);
        this.$emit('update:flowGraphProp', flowData);
...
//      },
    ...
    mounted() {
      this.$route.query.type !== 'edit' && this.initG6();
    },
    watch: {
      /**
       * 监听输入框
